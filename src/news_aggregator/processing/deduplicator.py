...
class Deduplicator:
    ...
    def __init__(self, history_file: Path, similarity_threshold: int = 85):
        """
    ...
    def deduplicate(self, articles: List[Article]) -> List[Article]:
        ...
        # Reset statistics
        self.stats = {'url_duplicates': 0, 'title_duplicates': 0, 'history_filtered': 0}
        ...
        self.logger.info(
            f"Deduplication complete: {initial_count} -> {final_count} articles "
            f"(removed: {self.stats['url_duplicates']} URL duplicates, "
            f"{self.stats['title_duplicates']} title duplicates, "
            f"{self.stats['history_filtered']} previously sent)"
        )
    ...
    def _deduplicate_by_url(self, articles: List[Article]) -> List[Article]:
        ...
        for article in articles:
            if article.url not in seen_urls:
                ...
                self.logger.debug(f"Removed URL duplicate: {article.url}")
    ...
    def _filter_sent(self, articles: List[Article]) -> List[Article]:
        ...
        for article in articles:
            if not self.history.is_sent(article.url):
                ...
                self.logger.debug(f"Filtered previously sent article: {article.url}")
